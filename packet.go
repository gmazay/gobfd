...
func DecodePacket(packetBytes []byte) (*layers.BFD, error) {
	...
	if p.ErrorLayer() != nil {
		// Unpacking failed
		////logger.Errorf("Failed to decode packet  %v", p.ErrorLayer().Error())
		return pbfd, errors.New("decode packet error")
	}

	// Make sure the package contains this layer:
	...
	// Successful decoding
	//logger.Debugf(PACKET_DEBUG_MSG, pbfd.Diagnostic, pbfd.State, pbfd.Poll, pbfd.Final,
...
func checkLayers(p gopacket.Packet, want []gopacket.LayerType) error {
	...
	if len(layers) < len(want) {
		////logger.Errorf(" Number of layers mismatch: got %d want %d", len(layers), len(want))
...
func validate(pbfd *layers.BFD) error {
	...
	// Other verification, Auth and other certifications to be added
	if pbfd.AuthPresent {
...
func EncodePacket(Version layers.BFDVersion,
	...
	if err != nil {
		////logger.Errorf("serial error: %s" + err.Error())
		return []byte{}
	}
